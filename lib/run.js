...
async function getModifiedFiles() {
    ...
    for (const file of response.data.files) {
        ...
        } else if (fileStatuses.includes(status)) {
...
async function run() {
    ...
    if (oneOrMoreWarningsFound || oneOrMoreErrorsFound) {
        ...
        output += `summary=clj-kondo took:${duration}ms, errors: ${error}, warnings: ${warning} (${allAnnotations.length} are new in changed files), info: ${info}\n`;
